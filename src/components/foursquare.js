...
export const requestFoursqureApi = (lat, lng) =>
    fetch(urlReq + lat + "," + lng, {})
        .then(res => res.json()).catch(function (err) { alert("Fouresquare content not available");window.location.reload();}) //if there is error in fethcing the promise
        .then(data => data).catch(function (err) { alert("Fouresquare content not available");window.location.reload();});
